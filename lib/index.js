exports.default = function (_ref) {
	...
	const traverser_ClassProperty = {
		ClassProperty: function(path, state) {
			...
			if (path.node.decorators[0].expression.name) { // if decorator has no args
				...
				var newExpression = t.callExpression(
					...
					path.node.decorators[0].expression,
					[t.identifier(state.classPath.node.id.name), t.stringLiteral(path.node.key.name)]
				);
				state.classPath.insertAfter(newExpression);
			}
			else if (path.node.decorators[0].expression.arguments) { // if decorator with args
				...
				var newExpression = t.callExpression(
					...
					path.node.decorators[0].expression.callee,
					[t.identifier(state.classPath.node.id.name), t.stringLiteral(path.node.key.name)].concat(path.node.decorators[0].expression.arguments)
				);
				state.classPath.insertAfter(newExpression);
			}
